    ...
    while position != contents.len() {
        match contents[position] {
            _ if contents[position].clone().is_alphabetic() => {
                ...
                position += 1;
                while position != contents.len() && (contents[position].is_alphanumeric() || contents[position] == '_') {
                    token.push(contents[position].clone());
                    position += 1;
                }
...
fn main() {
    ...
    let mut file = BufReader::new(File::open(filename).expect("Something went wrong reading the file"));
